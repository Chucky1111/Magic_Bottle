...
class ReaderBStateManager:
    """读者 B 状态管理器，负责维护读者 B 模块的状态"""
    
    def __init__(self, state_file_path: str = "data/reader_b_state.json"):
        """
        初始化读者 B 状态管理器
    ...
    def load_state(self) -> None:
        ...
        try:
            if self.state_file_path.exists():
                ...
                logger.info(f"读者 B 状态已从 {self.state_file_path} 加载")
                ...
                self.state = {
                    ...
                    "reader_history_file": "data/reader_b_history.json",  # 读者历史记录文件
                }
                self.save_state()
                logger.info(f"创建新的读者 B 状态文件: {self.state_file_path}")
        except Exception as e:
            logger.error(f"加载读者 B 状态失败: {e}")
            # 使用默认状态
            self.state = {
                ...
                "reader_history_file": "data/reader_b_history.json",
            }
    ...
    def save_state(self) -> None:
        ...
        try:
            ...
            logger.debug(f"读者 B 状态已保存到 {self.state_file_path}")
        except Exception as e:
            logger.error(f"保存读者 B 状态失败: {e}")
            raise
    ...
    def add_to_window(self, chapter_num: int) -> None:
        ...
        if chapter_num not in window:
            ...
            logger.debug(f"读者 B 窗口添加章节 {chapter_num}: {window}")
    ...
    def set_base_context_length(self, length: int) -> None:
        ...
        self.save_state()
        logger.debug(f"读者 B 基准上下文长度已设置为: {length}")
    
    def get_reader_history_file(self) -> str:
        """获取读者历史记录文件路径"""
        return self.state.get("reader_history_file", "data/reader_b_history.json")
