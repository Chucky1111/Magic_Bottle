...
class ChatWorkflow:
    ...
    def __init__(self):
        """初始化聊天工作流"""
        self.llm_client = LLMClient(config=settings.writer_llm.get_client_config())
        ...
        # 用户灵感管理器初始化
        self.user_idea_manager = get_user_idea_manager(
            state_manager=self.state_manager,
            config={
                "directory": settings.user_idea.directory,
                "completed_prefix": settings.user_idea.completed_prefix,
                "max_retries": settings.user_idea.max_retries,
                "enabled": settings.user_idea.enabled,
                "collaborative_mode": settings.user_idea.collaborative_mode,
                "auto_cc_enabled": settings.user_idea.auto_cc_enabled,
                "min_chapters_per_idea": settings.user_idea.min_chapters_per_idea
            }
        )
        
        # 新模块：历史管理、提示词渲染、上下文构建
        self.history_manager = HistoryManager()
        self.prompt_renderer = PromptRenderer(
            self.state_manager,
            self.system_info_manager,
            self.sequence_manager,
            self.prompt_logger,
            self.world_view_content,
            user_idea_manager=self.user_idea_manager
        )
    ...
    def _load_system_prompt(self) -> str:
        """加载系统提示词"""
        return self.prompt_renderer.load_system_prompt()
    
    def _load_feedback_prompt_template(self) -> str:
        """加载反馈提示词模板"""
        return self.prompt_renderer.load_feedback_prompt_template()
    
    def _load_prompt_template(self, prompt_file: str) -> str:
        """加载提示词模板文件"""
        return self.prompt_renderer.load_prompt_template(prompt_file)
    
    def _render_design_prompt(self) -> str:
        """渲染设计阶段提示词"""
        return self.prompt_renderer.render_design_prompt()
    
    def _render_simple_design_prompt(self) -> str:
        """渲染简化的设计阶段提示词（用于历史记录）"""
        return self.prompt_renderer.render_simple_design_prompt()
    
    def _render_simple_write_prompt(self) -> str:
        """渲染简化的写作阶段提示词（用于历史记录）"""
        return self.prompt_renderer.render_simple_write_prompt()
    
    def _render_write_prompt(self) -> str:
        """渲染写作阶段提示词"""
        return self.prompt_renderer.render_write_prompt()
    
    def _render_summary_prompt(self) -> str:
        """渲染总结阶段提示词"""
        return self.prompt_renderer.render_summary_prompt()
        ...
        try:
            ...
            if available_feedbacks:
                ...
                if feedback:
                    ...
                    # 索引信息需要从FeedbackManager获取，暂时设置为1
                    ...
                    logger.info(f"第{chapter_num}章没有可用的读者反馈（pop_feedback返回空）")
            else:
                logger.info(f"第{chapter_num}章没有可用的读者反馈")
                # 如果没有可用反馈，可以尝试使用上次的反馈（如果状态中有记录）
                last_feedback_chapter = self.state_manager.get_last_used_feedback_chapter()
                if last_feedback_chapter > 0:
                    logger.info(f"第{chapter_num}章使用上次的反馈（来自第{last_feedback_chapter}章），但实际无新反馈可注入")
        except Exception as e:
    ...
    def _execute_pruning_fallback(self, full_history: List[Dict[str, Any]],
        ...
        for i, msg in enumerate(full_history):
            ...
            # 跳过用户总结消息（只保留助手总结）
            if role == "user" and (stage == "summary" or is_summary):
                logger.debug(f"删除用户总结消息 {i}: 第{chapter}章 {stage}")
                continue
        ...
        # 处理总结消息：保留所有当前窗口的总结消息，如果没有则保留最新的总结消息
        if summary_messages:
            # 识别当前窗口的总结消息
            current_window_summaries = []
            other_summaries = []
            for idx, msg in summary_messages:
                msg_window_chapters = msg.get("window_chapters", [])
                if msg_window_chapters == window_chapters:
                    current_window_summaries.append((idx, msg))
                else:
                    other_summaries.append((idx, msg))
            
            if current_window_summaries:
                # 保留所有当前窗口的总结消息
                for idx, msg in current_window_summaries:
                    new_history.append(msg)
                    logger.debug(f"保留当前窗口总结消息 {idx}: 窗口章节{msg.get('window_chapters', [])}")
            else:
                # 没有当前窗口的总结，保留最新的总结消息（按时间戳排序）
                ...
                logger.debug(f"保留最新总结消息 {latest_summary_index}: 时间戳{latest_summary_msg.get('timestamp', 0)}")
    ...
    def write_chapter_to_file(self, title: str, body: str, chapter_num: int = None) -> str:
        ...
        if self.user_idea_manager and self.user_idea_manager.state_manager:
            ...
            if current_idea:
                file_name = current_idea.get("file_name")
                entry_index = current_idea.get("entry_index")
                if file_name and entry_index is not None:
                    # 构建文件路径
                    from pathlib import Path
                    idea_dir = Path(self.user_idea_manager.idea_dir)
                    file_path = idea_dir / file_name
                    
                    if file_path.exists():
                        success = self.user_idea_manager.mark_idea_completed(file_path, entry_index)
                        if success:
                            logger.info(f"已标记用户灵感条目为已完成: {file_name}[{entry_index}] (第{chapter_num}章)")
                        else:
                            logger.warning(f"标记用户灵感条目失败: {file_name}[{entry_index}]")
                    else:
                        logger.warning(f"用户灵感文件不存在: {file_path}")
                
                # 清除当前灵感记录，准备下一章
                self.state_manager.clear_current_user_idea()
    ...
    def run_summary_phase(self) -> None:
        """执行总结阶段（包含summary_c、summary_b和常规总结三次调用）"""
        logger.info("开始总结阶段")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self.user_idea_manager:
            try:
                stats = self.user_idea_manager.get_stats()
                ...
                user_idea_status = {
                    ...
                    "total_injected_entries": sum(len(indices) for indices in injections.values()) if injections else 0,
                    "has_pending_ideas": stats.get("pending_entries", 0) > 0
                }
                
                # 导出状态JSON到文件（用于监控）
                try:
                    self.user_idea_manager.export_status_json()
                except Exception as e:
                    logger.warning(f"导出用户灵感状态JSON失败: {e}")
            except Exception as e:
                logger.warning(f"获取用户灵感状态失败: {e}")
                user_idea_status = {
                    "enabled": True,
                    "error": str(e)
                }
