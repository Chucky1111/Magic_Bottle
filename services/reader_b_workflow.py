...
class ReaderBWorkflow:
    ...
    def __init__(self):
        ...
        self.state_manager = ReaderBStateManager()
        ...
        logger.info("ReaderBWorkflow初始化完成")
    ...
    def _initialize_system(self) -> None:
        ...
        if not reader_history_path.exists() or reader_history_path.stat().st_size == 0:
            logger.info("检测到读者 B 模块全新启动，开始运行读者预热流程...")
            
            # 运行读者预热
            self._run_reader_warmup(reader_history_path)
            
            logger.info("读者 B 预热完成，准备开始处理章节")
    ...
    def _run_reader_warmup(self, history_path: Path) -> None:
        ...
        if not questions:
            logger.warning("没有找到读者 B 预热问题，创建最小化历史记录")
        ...
        for i, question in enumerate(questions, 1):
            try:
                logger.info(f"运行读者 B 预热 Q{i}/{len(questions)}")
                ...
                logger.info(f"  读者 B 预热 Q{i} 完成，回答长度: {len(response)} 字符")
                ...
                logger.error(f"运行读者 B 预热 Q{i} 失败: {e}")
                continue
        ...
        logger.info(f"读者 B 预热流程完成，生成 {len(messages)} 条消息")
    ...
    def _create_minimal_reader_history(self, history_path: Path, system_content: str) -> None:
        ...
        logger.info("创建最小化读者 B 历史记录（仅系统消息）")
    ...
    def _save_reader_history(self, messages: List[Dict[str, Any]], history_path: Path) -> None:
        ...
        try:
            ...
            logger.info(f"读者 B 历史记录已保存到: {history_path} ({len(history)} 条消息)")
            
        except Exception as e:
            logger.error(f"保存读者 B 历史记录失败: {e}")
    ...
    def _load_reader_system_prompt(self) -> str:
        """加载读者系统提示词"""
        system_prompt_path = Path("prompts/reader_b/reader_system_prompt.txt")
        if not system_prompt_path.exists():
            logger.warning(f"读者 B 系统提示词文件不存在: {system_prompt_path}")
            return "你是一位真实的网文读者，喜欢阅读各种故事。"
        
        try:
            with open(system_prompt_path, 'r', encoding='utf-8') as f:
                content = f.read().strip()
            logger.debug(f"读者 B 系统提示词已加载，长度: {len(content)} 字符")
            return content
        except Exception as e:
            logger.error(f"加载读者 B 系统提示词失败: {e}")
            return "你是一位真实的网文读者，喜欢阅读各种故事。"
    
    def _parse_reader_warmup_questions(self) -> List[str]:
        """解析读者预热问题"""
        warmup_file_path = Path("prompts/reader_b/reader_warmup.md")
        if not warmup_file_path.exists():
            logger.warning(f"读者 B 预热文件不存在: {warmup_file_path}")
            return []
        ...
        try:
            ...
            logger.info(f"从读者 B 预热文件解析出 {len(questions)} 个问题")
            return questions
            
        except Exception as e:
            logger.error(f"解析读者 B 预热文件失败: {e}")
            return []
    
    def _load_prompt_template(self, prompt_file: str) -> str:
        """加载提示词模板文件"""
        prompt_path = Path("prompts/reader_b") / prompt_file
        if not prompt_path.exists():
            # 尝试在prompts目录下查找
            prompt_path = Path("prompts") / prompt_file
            if not prompt_path.exists():
                raise FileNotFoundError(f"读者 B 提示词文件不存在: {prompt_file}")
...
<other>其它内容</other>
    ...
    def _run_memory_packet(self, chapter_num: int) -> str:
        ...
        Returns:
            整理后的记忆内容
        """
        logger.info(f"开始整理读者 B 记忆 - 第{chapter_num}章，当前记忆长度: {len(self.state_manager.get_reader_memory())} 字符")
        ...
        if parsed.get("success", False):
            ...
            logger.info(f"读者 B 记忆整理完成，整理后长度: {len(packed_memory)} 字符")
            return packed_memory
        else:
            logger.warning("读者 B 记忆整理解析失败，返回原始记忆")
    ...
    def _get_messages_for_llm(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.error(f"加载读者 B 历史记录失败: {e}")
            return []
        ...
        logger.debug(f"构建了 {len(messages)} 条读者 B 消息给LLM")
        return messages
    ...
    def _add_to_reader_history(self, role: str, content: str, chapter_num: int, stage: str) -> None:
        ...
        try:
            ...
            logger.debug(f"读者 B 历史记录已添加: {role} - 第{chapter_num}章 - {stage}")
            
        except Exception as e:
            logger.error(f"保存读者 B 历史记录失败: {e}")
    
    def _save_memerry_to_file(self, memerry_content: str) -> None:
        """保存读者记忆到prompts/reader_b/memerry.txt文件（覆盖写入）"""
        memerry_path = Path("prompts/reader_b/memerry.txt")
        ...
        try:
            ...
            logger.info(f"读者 B 记忆已保存到: {memerry_path}, 长度: {len(memerry_content)} 字符")
            
        except Exception as e:
            logger.error(f"保存读者 B 记忆失败: {e}")
    
    def _save_feedback_to_file(self, feedbacks: List[str], chapter_num: int) -> None:
        """保存反馈到data/feedback_b.txt文件，使用 --- 分隔符"""
        feedback_path = Path("data/feedback_b.txt")
        ...
        try:
            ...
            if existing_feedback:
                existing_sections = [s.strip() for s in existing_feedback.split('---') if s.strip()]
            ...
            for fb in feedbacks:
                ...
                # 可选：在反馈内容前添加章节标记
                # 但为了保持简洁，我们只保留原始反馈内容
                new_sections.append(fb_clean)
            
            # 合并所有反馈（现有 + 新反馈）
            all_sections = existing_sections + new_sections
            
            # 用 --- 连接所有部分
            # 每个部分之间用空行和 --- 分隔，以提高可读性
            separator = '\n\n---\n\n'
            new_content = separator.join(all_sections)
            
            # 写入文件
            with open(feedback_path, 'w', encoding='utf-8') as f:
                f.write(new_content)
            
            logger.info(f"读者 B 反馈已保存到: {feedback_path}, 新增 {len(new_sections)} 条反馈，来自第{chapter_num}章，总计 {len(all_sections)} 条反馈")
            
        except Exception as e:
            logger.error(f"保存读者 B 反馈失败: {e}")
    ...
    def run_memerry_phase(self, chapter_content: str, chapter_num: int) -> None:
        ...
        if parsed.get("success", False):
            ...
            if len(combined_memory) >= 2000:
                logger.info(f"读者 B 记忆达到 {len(combined_memory)} 字符，触发记忆整理")
                ...
                # 保存记忆到文件 prompts\reader_b\memerry.txt（覆盖写入）
    ...
    def _execute_pruning(self) -> None:
        ...
        4. 保留最新的反馈消息（如果有）
        """
        logger.info("开始执行读者 B 历史记录修剪逻辑")
        
        # 获取当前窗口章节
        window_chapters = self.state_manager.get_window_chapters()
        if len(window_chapters) != 3:
            logger.error(f"读者 B 窗口章节数量不正确: {window_chapters}，应为3章")
            return
        ...
        history_path = Path(self.state_manager.get_reader_history_file())
        if not history_path.exists():
            logger.warning("读者 B 历史记录文件不存在，跳过修剪")
            return
        ...
        for i, msg in enumerate(full_history):
            ...
            if i < base_context_length:
                ...
                logger.debug(f"保留读者 B 基准上下文消息 {i}: {msg.get('stage', 'base')}")
                continue
            ...
            if chapter == chapter_3:
                ...
                if stage == "packet":
                    logger.debug(f"删除读者 B packet阶段消息 {i}: 第{chapter}章 {stage}")
                    continue  # 跳过，不添加
                new_history.append(msg)
                logger.debug(f"保留读者 B 第三章消息 {i}: 第{chapter}章 {stage}")
                continue
            
            # 情况3：删除前两章的所有记录
            if chapter in chapters_to_remove:
                logger.debug(f"删除读者 B 前两章消息 {i}: 第{chapter}章 {stage}")
                continue  # 跳过，不添加
            ...
            # 这样可以确保上下文干净
            logger.debug(f"删除读者 B 其他消息 {i}: 第{chapter}章 {stage}")
        ...
        try:
            ...
            logger.info(f"读者 B 历史记录修剪完成: 删除了章节 {chapters_to_remove} 和所有packet阶段消息，保留章节 {chapter_3}")
            logger.info(f"读者 B 历史记录长度: {len(full_history)} -> {len(new_history)}")
            
        except Exception as e:
            logger.error(f"保存修剪后的读者 B 历史记录失败: {e}")
    ...
    def run_step(self) -> bool:
        ...
        try:
            ...
            logger.info(f"开始处理第{chapter_num}章 - 读者 B 模块")
            
            current_stage = self.state_manager.get_stage()
            logger.info(f"读者 B 当前状态: {self.state_manager.get_state_summary()}")
            ...
            if current_stage == "memerry":
                ...
                logger.error(f"未知的读者 B 阶段: {current_stage}")
                return False
            
            return True
            
        except Exception as e:
            logger.error(f"执行读者 B 步骤时发生错误: {e}", exc_info=True)
            return False
...
def main():
    ...
    workflow = ReaderBWorkflow()
    
    # 显示状态
    status = workflow.get_status()
    print(f"读者 B 状态: {status['state']}")
    print(f"最后处理章节: {status['last_processed_chapter']}")
    print(f"当前阶段: {status['current_stage']}")
    print(f"读者 B 记忆长度: {status['reader_memory_length']} 字符")
    ...
    if chapter_info:
        ...
        # 询问是否运行读者模块
        response = input("\n是否运行读者 B 模块处理此章节？ (y/N): ").strip().lower()
        if response == 'y':
            print("\n开始运行读者 B 模块...")
